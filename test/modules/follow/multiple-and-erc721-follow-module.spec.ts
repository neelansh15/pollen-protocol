...
makeSuiteCleanRoom('Multiple AND ERC721 Gated Follow Module', function () {
  ...
  context('Negatives', function () {
    ...
    context('Processing follow', function () {
      it.only('User should fail to process follow if the user does not own all the NFTs', async function () {
      ...
      it.only('User should fail to follow if the user owns some but not all of the NFTs', async function () {
        const data = abiCoder.encode(
          ['address[]'],
          [[myNFT.address, myNFT2.address, myNFT3.address]]
        );
